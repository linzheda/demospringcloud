...
public class UserController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "user", value = "登录用户信息", required = true, dataType = "User")
    })
    @RequestMapping(value = "/login", method = RequestMethod.POST)
    @PassToken
    public @ResponseBody
    ResultUtil login(@RequestBody User user) {
        ...
        try {
            user = userservice.getUser(user.getName(), md5Password);
    ...
    public @ResponseBody ResultUtil getUserInfo(Integer id) {
    ...
    public @ResponseBody ResultUtil getUserInfo2(@PathVariable Integer id){
